...
export default defineComponent({
  ...
  methods: {
    getGenres(genre: number[], page: number = 1) {
      return this.storeMovies.methods.getGenres(genre, page);
    },
    ...
    async handleScroll(event: any) {
      ...
      if (
        ...
        target.scrollHeight - 400 * this.currentPage
      ) {
        if (!this.end && !this.isLoadingNextPage) {
          this.isLoadingNextPage = true;
          const category = this.currentTab || null;
          if (category)
            await this.getGenres(category.value, this.currentPage + 1);
  ...
  mounted() {
    ...
    if (category) this.getGenres(category.value, this.currentPage);
  },
});
</script>
